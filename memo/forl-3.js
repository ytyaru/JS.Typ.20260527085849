...
export const getFORL = (currentPath = '') => {
  ...
  const handler = {
    get(target, key) {
      ...
      const nextPath = target.$.path ? `${target.$.path}.${key}` : key;
...
console.log(res1.$); // { path: "p.num.int", args: null }
console.log(res2.$); // { path: "p.num.int.bit", args: [8] }
console.log(res3.$); // { path: "p.str.some", args: ["A", "B"] }
