...
a.r.ins = (v,...Cs) => {
    Cs.forEach(C=>{if(!isFn(C)){throw new Error(`r.insの第二引数以降は期待する型(コンストラクタ関数)であるべきです。:${Cs},${typeof Cs}`)}});
    const isIns = a.r(v) && !a.r.cls(v) && !a.r.cal(v) && !a.r.ary(v) && !a.r.obj(v) && !a.r.dic(v) && !a.r.des(v);
    if (!isIns) return false;
    if (0===Cs.length) return true;
