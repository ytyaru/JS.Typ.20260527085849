...
*/
export const typOf = (isThrow, v,...cbs) => {
    const res = getForls(v, ...cbs);
    const R = getNestRes(res);
    if (isThrow && !R) {
...
//        throw new TypeError(`値が期待する型と違います。期待:${cbs.join(',')}, 実際:${getId(v, a.r.ins(v) ? v.constructor : (a.r.cls(v) ? v : undefined))}`)
//        throw new TypeError(`値が期待する型と違います。期待:${forls.map(f=>f.$.path).join(',')}, 実際:${getId(v, a.r.ins(v) ? v.constructor : (a.r.cls(v) ? v : undefined))}`)
    }
    return R;
};
const getForls = (v, ...cbs) => {
    const res = {forls:null, result:null, children:null};
    if (!cbs.every(cb=>'function'===typeof cb)) {throw new Error(`引数不正。第二引数以降はa=>a.pなどのコールバック関数であるべきです。`)}
//    const forls = cbs.map(cb=>cb(getFORL()));
